define([
  ...
  "plugins/dialog",
  "./recop",
], function (app, ko, router, dialog, recop) {
  ...
  editoffer.prototype.attached = function () {
    ...
    offerSub1(
      strid,
      "C3_534182272208='" +
        editdata.C3_589650598855 +
        "' AND C3_534182291688='" +
        editdata.C3_534181718652 +
        "'",
      function (data) {
        me.subList2(data);
      }
      ...
      setTimeout(function () {
        recop.show().then(function (opn) {
          if (opn) {
            ...
            offerSub1(
              strid,
              "C3_534182272208='" + editdata.C3_589650598855 + "'",
              function (data) {
                me.subList2(data);
              }
            );
            offerSub1(
              aveid,
              "C3_534182432109='" + editdata.C3_534181730034 + "'",
              function (data) {
                me.subList4(data);
              }
            );
    ...
    edept1 = function () {
      var where =
        "C3_419448436728 = '" +
        dp1 +
        "' and C3_417728131816=1 and DEP_ADMIN_ID>0";
    ...
    uploadFileChange = function (input) {
      var uploadFile = function (file, url, mode) {
      ...
      if (input.files && input.files[0]) {
        ...
        uploadFile(
          ...
          .then(function (url) {
            var img = $("#imgUploaded");
            var a3 = $("#ahref");
            var imgfield3 = mini.get("imgurl");
            imgfield3.setValue(url);
    ...
    uploadFileChange2 = function (input) {
      var uploadFile = function (file, url, mode) {
        return new Promise(function (resolve, reject) {
          let fd = new FormData();
          fd.append("file", file, file.name);
      ...
      if (input.files && input.files[0]) {
        ...
        uploadFile(
          ...
          .then(function (url) {
            ...
            var a3 = $("#ahref2");
            var imgfield3 = mini.get("imgurl2");
            imgfield3.setValue(url);
            img[0].src = "../../img/timg.jpg";
            img[0].width = "60";
            a3[0].href = url;
    ...
    uploadFileChange3 = function (input) {
      var uploadFile = function (file, url, mode) {
        return new Promise(function (resolve, reject) {
          let fd = new FormData();
          fd.append("file", file, file.name);
